...
"""
######################################################
##### LSTM2 (2 hidden layers) using BGP datasets #####
######################################################
...
class RNN(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(input_lstm, hidden_size, num_layers, batch_first=False, dropout=0.4, bidirectional=True)
    ...
    def forward(self, x):
        ...
        if torch.cuda.is_available():
            ...
            h0 = Variable(torch.zeros(self.num_layers*2, x.size(1), self.hidden_size))
            c0 = Variable(torch.zeros(self.num_layers*2, x.size(1), self.hidden_size))
...
model_pkl = 'birnn-lstm2-%d' % sequence_length
torch.save(rnn.state_dict(), '%s.pkl'%model_pkl)
